...
## v3.31.0（全量代码审核修复：数据层迁移补强 / 引擎桥接补齐 / 阅读页缺陷与未完成功能）
- **数据层**：`txt_toc_rules` 建表缺 `enable` 列导致老库读取崩溃，Version 升至 9 并补 `ALTER TABLE` 迁移；清理死列、书籍删除时级联清理章节/书签/阅读记录、批量写入改为事务。
- **阅读记录**：修复会话时长与翻页计数语义混用、统计口径被自动阅读/朗读污染的问题。
- **JS 桥接层**：`java.md5Encode/md5` 由占位改为接通文件内 RFC 1321 实现（返回小写 hex，对齐 legado JsExtensions）；`java.createSymmetricCrypto(key, mode, iv)` 由抛错改为返回真实 AES 工厂对象（`encrypt/encryptBase64/decrypt/decryptStr`，基于已注入的 aes-js + PKCS7，支持 CBC/ECB）；复核 `java.connect` 响应头回填链路闭环。
- **阅读页**：章节加载失败不再把错误文案写入正文缓存，改为展示「本章加载失败 + 重试」视图；页脚时间/电量每分钟定时刷新，电量经 battery_plus 实时读取（新增依赖，读取失败回退静态图标）；`pageAnim` 翻页动画接入 PageView（滚动/上下为纵向、无动画禁用滑动手势，覆盖/仿真近似为默认滑动，需真机打磨）。
- **设置页 Web 服务**：自动恢复启动失败时不再静默吞错，改为关闭开关并提示；Web 服务说明弹窗移除硬编码 `http://localhost:1122`，统一显示实际局域网 IP 并在未运行时给出降级说明。
- 已知限制：沙箱无 Flutter SDK，本批修改未经编译与真机验证；battery_plus 为新增原生依赖，需 `flutter pub get` 后随宿主打包。
