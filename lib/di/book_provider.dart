...
class ReadProvider extends ChangeNotifier {
  static const String kConfigKey = 'read_config_json';
  ...
  /// 从 SharedPreferences 恢复阅读配置（应用启动时调用一次）
  Future<void> load() async {
    if (_loaded) return;
    _loaded = true;
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(kConfigKey);
      if (raw != null && raw.isNotEmpty) {
        _config = ReadConfig.fromJsonString(raw);
        notifyListeners();
      }
    } catch (_) {
      // 配置损坏时回退默认值，不阻塞启动
  ...
  Future<void> _persist() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(kConfigKey, _config.toJsonString());
    } catch (_) {}
  }
  ...
  void setTextSize(int size) => updateConfig((c) => c.textSize = size);

  void setBgColor(int color) => updateConfig((c) => c.bgColor = color);

  void setTextColor(int color) => updateConfig((c) => c.textColor = color);

  void setPageAnim(int anim) => updateConfig((c) => c.pageAnim = anim);

  void setLineSpacing(int spacing) => updateConfig((c) => c.lineSpacing = spacing);
}
